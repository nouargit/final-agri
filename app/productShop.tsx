...
export default function ProductShopScreen() {
  ...
  // Get shop_id from params
  const { shop_id } = useLocalSearchParams();
  
  // Set selectedShopId only once when component mounts or shop_id changes
  useEffect(() => {
  ...
  const getShopProducts =async () =>{
    ...
    const response = await fetch(`${config.baseUrl}/api/products?shop_id=${selectedShopId}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Accept': 'application/json',
    ...
    if (response.ok) {
        ...
        setShopItems(products.data || products);
